 ...
 */
package org.jf.smalidea.codeInsight.daemon.impl.quickfix.unicode;
 ...
 */
public class ReplaceOctalEscapeWithUnicodeEscapeIntention extends SmaliIntention {
    ...
    private static class OctalEscapePredicate extends PsiElementEditorPredicate {
        ...
        public boolean satisfiedBy(PsiElement element, @Nullable Editor editor) {
            ...
            } else if (PsiUtil.isSmaliToken(element, SmaliTokens.STRING_LITERAL)) {
                final LeafPsiElement literalExpression = (LeafPsiElement) element;
                final String text = literalExpression.getText();
                final CaretModel model = editor.getCaretModel();
                final int offset = model.getOffset() - literalExpression.getTextOffset();
