 ...
 */
package org.jf.smalidea.codeInsight.daemon.impl.quickfix;
...
public class ConvertToStringLiteralAction implements IntentionAction {
    ...
    public boolean isAvailable(@NotNull final Project project, final Editor editor, final PsiFile file) {
        ...
        return PsiUtil.isSmaliToken(element, SmaliTokens.STRING_LITERAL);
    }

    @Override
    public void invoke(@NotNull final Project project, final Editor editor, final PsiFile file) throws IncorrectOperationException {
        final PsiElement element = file.findElementAt(editor.getCaretModel().getOffset());
        if (element != null && PsiUtil.isSmaliToken(element, SmaliTokens.STRING_LITERAL)) {
            ...
            if (length > 1 && text.charAt(0) == '\"' && text.charAt(length - 1) == '\"') {
                ...
                if (literal != null && PsiUtil.isSmaliToken(literal, SmaliTokens.STRING_LITERAL)) {
