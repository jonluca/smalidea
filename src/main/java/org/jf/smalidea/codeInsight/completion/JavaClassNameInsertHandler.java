...
* @author peter
*/
class JavaClassNameInsertHandler implements InsertHandler<SmaliPsiClassReferenceElement> {
  static final InsertHandler<SmaliPsiClassReferenceElement> JAVA_CLASS_INSERT_HANDLER = new JavaClassNameInsertHandler();
  ...
  public void handleInsert(final InsertionContext context, final SmaliPsiClassReferenceElement item) {
    ...
    if (item.getUserData(JavaChainLookupElement.CHAIN_QUALIFIER) == null &&
      ...
      if (shouldHaveAnnotationParameters(psiClass)) {
        SmaliCompletionUtil.insertParentheses(context, item, false, true);
      }
    ...
    if (fillTypeArgs && context.getCompletionChar() != '(') {
      SmaliCompletionUtil.promptTypeArgs(context, context.getOffset(refEnd));
    }
  }

  private static boolean shouldInsertFqnInJavadoc(@NotNull SmaliPsiClassReferenceElement item,
