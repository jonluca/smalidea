...
import org.jf.smalidea.psi.impl.search.SmaliHighlightingCaches;
import org.jf.smalidea.psi.index.SmaliSuperClassNameOccurenceIndex;
 ...
 */
public class SmaliDirectClassImplementationsSearch implements QueryExecutor<PsiClass, DirectClassInheritorsSearch.SearchParameters> {

    @Override
    public boolean execute(@NotNull DirectClassInheritorsSearch.SearchParameters parameters, @NotNull Processor<? super PsiClass> consumer) {
        ...
        if (org.jf.smalidea.util.PsiUtil.isJavaLangObject(baseClass)) {
            ...
            return AllClassesSearch.search(useScope, project).forEach(psiClass -> {
                ...
                return superClass == null || !org.jf.smalidea.util.PsiUtil.isJavaLangObject(superClass) || consumer.process(psiClass);
            });
        }
    ...
    private static PsiClass[] getOrCalculateDirectSubClasses(@NotNull Project project, @NotNull PsiClass baseClass, @NotNull SearchScope useScope) {
        ConcurrentMap<PsiClass, PsiClass[]> map = SmaliHighlightingCaches.getInstance(project).DIRECT_SUB_CLASSES;
    ...
    private static PsiClass[] calculateDirectSubClasses(@NotNull Project project,
        ...
        GlobalSearchScope globalUseScope = dumbService.runReadActionInSmartMode(new Computable<GlobalSearchScope>() {
            @Override
            public GlobalSearchScope compute() {
                return GlobalSearchScopeUtil.toGlobalSearchScope(useScope, project);
            }
        });


        Collection<PsiReferenceList> candidates =
                dumbService.runReadActionInSmartMode(() -> SmaliSuperClassNameOccurenceIndex.getInstance().get(baseClassName, project, globalUseScope));
