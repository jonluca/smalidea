...
public class SmaliHighlightingCaches {

    public static SmaliHighlightingCaches getInstance(Project project) {
        return ServiceManager.getService(project, SmaliHighlightingCaches.class);
    }

    private final List<Map<?, ?>> allCaches = ContainerUtil.createConcurrentList();

    public SmaliHighlightingCaches(@NotNull Project project) {
    ...
    public final ConcurrentMap<PsiClass, PsiClass[]> DIRECT_SUB_CLASSES = createWeakCache();
    // baseClass -> all sub classes transitively, including anonymous
    public final ConcurrentMap<PsiClass, Iterable<PsiClass>> ALL_SUB_CLASSES = createWeakCache();
    // baseMethod -> all overriding methods
    public  final Map<PsiMethod, Iterable<PsiMethod>> OVERRIDING_METHODS = createWeakCache();
